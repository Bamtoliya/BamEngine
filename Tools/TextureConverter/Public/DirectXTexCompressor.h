...
public:
	...
	virtual bool GetRawCompressedData(const std::wstring& inputFilePath, std::vector<uint8>& outData, TextureBinaryHeader& header, Engine::ETextureFormat targetFormat = Engine::ETextureFormat::BC7_RGBA_UNORM, ECompressFlags flags = ECompressFlags::None) override;
};
END